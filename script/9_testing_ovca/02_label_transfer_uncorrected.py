...
for major_cell_type in major_cell_types:
    ...
    adata_path = ooseDir +  f'integrated_query_seacells_scarches_tissuetreat_{major_cell_type}_raw.h5ad'
    ...
    metastasis = sc.read_h5ad(mcDir + f'adata_metastasis_embeddings_cellstates.h5ad')
    ...
    adata.obs_names = ["-".join(n.split("-")[0:-2]) if n.endswith('MSK') else n for n in adata.obs_names]
    ...
    #adata.obs['cell_states'] = metastasis.obs['cell_states']
    ...
    # Save adata
    adata.write_h5ad(ooseDir + f'integrated_query_seacells_scarches_tissuetreat_predicted_cellstates_{major_cell_type}_raw.h5ad')
