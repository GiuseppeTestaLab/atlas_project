...
major_cell_types = ["fibroblasts", "immune", "endothelial", "cancer"]
...
for major_cell_type in major_cell_types:
    ...
    sc.settings.figdir = figPath + f"oose/{major_cell_type}_testing_ovca/"
    ...
    for name, ad in ad_by_tissue.items():
        ad = ad.raw.to_adata()
        ad = ad[:,hdgs]
        # sc.pp.log1p(ad)
        ...
        adata_target = adata_target[adata_target.obs['tissue'] == name.capitalize()]
        ...
        sc.pl.umap(integrated_query, color='14_paper_ID', frameon=False, save=f'oose_patient_latent_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='02_tissue', frameon=False, save=f'oose_tissue_latent_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='03_treatment', frameon=False, save=f'oose_treatment_latent_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='tissue-treatment', frameon=False, save=f'oose_tissue-treatment_latent_{major_cell_type}_{name}.png')
        
        ## Plot corrected gene expression space of integrated query and reference
        sc.pp.neighbors(integrated_query)
        sc.tl.umap(integrated_query)
        
        sc.pl.umap(integrated_query, color='14_paper_ID', frameon=False, save=f'oose_patient_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='02_tissue', frameon=False, save=f'oose_tissue_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='03_treatment', frameon=False, save=f'oose_treatment_{major_cell_type}_{name}.png')
        sc.pl.umap(integrated_query, color='tissue-treatment', frameon=False, save=f'oose_tissue-treatment_{major_cell_type}_{name}.png')

        
        if not os.path.exists(ooseDir):
            os.makedirs(ooseDir)

        integrated_query.write_h5ad(ooseDir + f'integrated_query_seacells_scarches_tissuetreat_{major_cell_type}_{name}_raw_corrected.h5ad')
