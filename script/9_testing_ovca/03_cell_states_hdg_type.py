...
for major_cell_type in major_cell_types:
    ...
    def write_degs(degs, path):
        for cluster in degs:
            degs[cluster].to_csv(f"{path}_{cluster}_de_hdg_type.csv")
    ...
    enrichment = {}
    for cluster in both:
        query = wrap_gprof(ranks_query ,cluster, hdgs)
        ref = wrap_gprof(ranks_ref, cluster, hdgs)
    ...
    overlap_df.to_csv(f"enrichment/{major_cell_type}_overlap_hdg_type.csv")
