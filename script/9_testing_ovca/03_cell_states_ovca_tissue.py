...
for major_cell_type in major_cell_types:
    ...
    for tissue_file in tissues:
        ...
        adata = ooseDir + f'integrated_query_seacells_scarches_tissuetreat_predicted_cellstates_{major_cell_type}_{tissue_file}_raw.h5ad'
        ...
        def write_degs(degs, path):
            for cluster in degs:
                degs[cluster].to_csv(f"{path}_{cluster}_de_ovca_tissue.csv")
        os.makedirs(f"enrichment/degs/{major_cell_type}", exist_ok=True)
        write_degs(ranks_query, f"enrichment/degs/{major_cell_type}/query_raw_tissue")
        write_degs(ranks_ref, f"enrichment/degs/{major_cell_type}/ref_raw_tissue")
        ...
        overlap_df.to_csv(f"enrichment/{major_cell_type}_{tissue_file}_overlap_ovca_tissue.csv")
