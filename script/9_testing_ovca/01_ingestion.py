...
major_cell_type = major_cell_types[0]
tissue = "metastasis"
...
sc.settings.figdir = figPath + f"oose/{major_cell_type}_testing_ovca_ingestion/"
#%%
...
mask = ad.obs['01_major_celltypes'] == "HematopoieticMSK"
ad = ad[~mask]
...
ad_by = adata_by_celltype_tissue(ad)
#%%
for tissue_dict in ad_by.values():
...
#%%
colors = ['07_cell_states']
for celltype, tissue_dict in ad_by.items():
    for tissue, adata in tissue_dict.items():
        sc.pl.umap(adata, color=colors, title=f"{celltype} {tissue}")
        sc.pl.embedding(adata, basis="umap_shifted", color=colors)

sc.pl.embedding(ad, basis="umap_shifted", color=colors)
# sc.pp.log1p(ad)
#%%
...
sc.tl.umap(ad)
sc.pl.umap(ad, color=['14_paper_ID', 'tissue-treatment', '02_tissue', '03_treatment', '07_cell_states'])
#%%
