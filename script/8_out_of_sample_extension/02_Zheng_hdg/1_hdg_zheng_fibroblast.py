...
adata_fibroblast = create_fibroblast_adata(adata)

adata_fibroblast.write(initDir + 'zheng2023_adata_fibroblast.h5ad')
...
for j in dataName:
    path = (dataDir + j.capitalize() + '/Adata' + "/{}_adata_fibroblast.h5ad").format(j)
    print(path)
    adata_fibroblast = sc.read(path)
    dispersion_gene_xpatient = {}
    highly_variable_genes_per_patient = {}
    for patient_code in adata_fibroblast.obs["paper_ID"].unique():
        patient_anndata = adata_fibroblast[adata_fibroblast.obs["paper_ID"] == patient_code].copy()
...
dispersion_table.to_csv(scriptsPath + '4_hdg/Tables/dispersion_table_fibroblast_zheng.csv')
hvg_table.to_csv(scriptsPath + '4_hdg/Tables/hvg_table_fibroblast_zheng.csv')

# 2_hvg_union_patients_dispersion_fibroblast zheng
## Computing HDG by patient based on dispersion values

dir = scriptsPath + '4_hdg/Tables/'
#%%
dispersion_table = pd.read_csv(scriptsPath + '4_hdg/Tables/dispersion_table_fibroblast_zheng.csv', index_col=0)
...
#%%
hdg_table.to_csv(dir + 'atlas_hdg_common_dispersion_patients_fibroblast_zheng.csv')

#%%
hdg_true = pd.DataFrame(index=list_of_genes)
hdg_true['highly_variable'] = "True"
hdg_true.to_csv(dir + 'atlas_hdg_dispersion_patients_fibroblast_zheng.csv')
