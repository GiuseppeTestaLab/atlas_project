...
singularity exec --nv -B $bindPaths -H $homePath $image \
                 /bin/bash -c "eval \"\$(conda shell.bash hook)\" && conda activate scgen && python3 ${scriptsPath}6_integration/metacells/endothelial/scGen_labels_tissuetreatment_2500.py"
