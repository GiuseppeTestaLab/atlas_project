...
singularity exec --nv -B $bindPaths -H $homePath $image \
                 /bin/bash -c "eval \"\$(conda shell.bash hook)\" && conda activate scvi && python3 ${scriptsPath}7_downstream/LR_interactions/Liana_aggregaterank_fibroblasts_endothelial.py"