...
#SBATCH --job-name=global_embeddings
#SBATCH --mem=200GB
...
singularity exec -B $bindPaths -H $homePath $image \
                 /bin/bash -c "eval \"\$(conda shell.bash hook)\" && conda activate downstream && python3 ${scriptsPath}7_downstream/clustering/global_embeddings.py"