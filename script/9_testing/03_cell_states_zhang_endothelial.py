...
path = "/group/testa/Project/OvarianAtlas/atlas_project/raw_data/out_of_sample_extension_backup/out_of_sample_extension/endothelial/integrated_query_zheng_seacells_scarches_tissuetreat_predicted_cellstates.h5ad"
...
new_dataset_adata.obs["predicted_cell_states"] = new_dataset_adata.obs["predicted_cell_states"].astype("category")
#%%
all_categories = pd.api.types.union_categoricals([
    adata.obs['cell_states'],
    new_dataset_adata.obs["predicted_cell_states"]
    ]).categories

# Align the categories of 'ascites_state' and 'metastasis_state' to match the combined categories
adata.obs['cell_states'] = adata.obs['cell_states'].cat.set_categories(all_categories)
new_dataset_adata.obs["predicted_cell_states"] = new_dataset_adata.obs["predicted_cell_states"].cat.set_categories(all_categories)
adata.obs.loc[new_dataset_adata.obs_names, "cell_states"] = new_dataset_adata.obs["predicted_cell_states"]

#%%

adata_ref = adata[~(adata.obs["dataset"] == "Zheng")]
adata_query = adata[adata.obs["dataset"] == "Zheng"]
adata_query.obs["predicted_cell_states"] = adata_query.obs["cell_states"]
#%% New Strategy
...
for tissue in both:
    ranks_query[tissue] = {}
    for cell_state in adata_query_by_tissue[tissue].obs.cell_states.cat.categories:
        adata_ref_test = adata_ref_by_tissue[tissue][adata_ref_by_tissue[tissue].obs.cell_states != cell_state]
        adata_test = sc.concat([adata_ref_test, adata_query_by_tissue[tissue][adata_query_by_tissue[tissue].obs.cell_states == cell_state]])
        sc.tl.rank_genes_groups(adata_test, groupby="cell_states", method="wilcoxon", groups=[cell_state])
...
overlap_df.to_csv("zheng_endothelial_overlap_filter.csv")
# %%
